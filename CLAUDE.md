...
安卓記帳 PWA：手動記帳 + 台灣電子發票 QR Code 掃描記帳。離線優先（IndexedDB），選用 Google Sheet + Apps Script 做雲端推送備份/拉取還原。**不是 APK**——手機透過 Chrome「加入主畫面」使用，沒有原生打包/發布流程。本機（Windows 開發機）沒有 Java/Android SDK/Gradle/Flutter，這是刻意選擇 PWA 路線的原因。
...
- `js/sync.js` — Apps Script push/pull 客戶端
- `js/app.js` — 路由 + 所有畫面渲染邏輯
- `lib/jsqr.min.js` — 內嵌（非 CDN）的 jsQR 備援解碼器，來源 unpkg `jsqr@1.4.0`，離線快取需要它
- `Code.gs` / `SETUP-GUIDE.md` — 雲端同步後端原始碼與部署文件，使用者自己貼上部署
- `manual.html` — 使用手冊

## IndexedDB 設計決策

- `entries.id` 用 `crypto.randomUUID()`（client-side 產生），不是自增 ID——這樣本機新增的紀錄和從雲端拉回來的紀錄不會撞 ID，同步時可以直接用 id 比對。
- 沒有獨立的「待同步佇列」表：「這筆需要同步」＝ `updatedAt > (syncedAt || 0)`，用計算代替維護一份容易跟資料本身失去同步的佇列。
- 刪除是軟刪除（`deleted` 標記 + 更新 `updatedAt`），不是真的從 IndexedDB 移除——這樣同步到雲端時，其他裝置的「拉取還原」才知道要把這筆也標記刪除，而不是誤以為它從沒同步過又拉回來。
...
`service-worker.js` 的 `CACHE_NAME` 是版本化的（目前 `expense-tracker-v1`）。**改動任何被快取的檔案（`SHELL_FILES` 清單裡的檔案）之後，一定要把 `CACHE_NAME` 升版**，否則手機上的舊快取不會更新——這是本工作區 `ai-course-hub` 已經踩過的坑（改了資料但瀏覽器還吃舊版 JS），這裡從一開始就用版本化快取名稱處理。同步 API（Apps Script 呼叫）在 `fetch` handler 裡依 origin 排除，一律 network-only，不進快取。
...
## 雲端同步（`Code.gs` + `js/sync.js`）

設計沿用工作區 `member-license-gate` skill 已驗證過的骨架慣例：用表頭文字比對 Google Sheet 欄位（不寫死欄位順序）、`LockService` 避免多裝置同時寫入互相覆蓋、前端 `fetch()` 不能加自訂 headers（否則觸發 CORS 預檢，Apps Script 不回應 OPTIONS）、測 `doPost` 要用 Node `fetch()` 不能用 `curl -L`（會把 POST 降級成 GET）。詳細踩坑見該 skill 的 `references/pitfalls-and-testing.md`。

同步策略是 push（上傳本機待同步紀錄）+ pull（下載雲端有更新的紀錄），以 `updatedAt` 較新者為準（last-write-wins），不是即時多裝置同步。Apps Script 部署網址存在 IndexedDB 的 `settings`，不寫死在程式碼——且這組網址等同存取金鑰（部署設為「任何人皆可存取」），`SETUP-GUIDE.md` 裡有明確告知使用者這件事。`Code.gs` 的實際部署（貼程式碼、按部署、過 OAuth 同意畫面）需要使用者自己動手，無法用瀏覽器自動化代勞。
...
- 沒有做真正的即時多裝置同步（例如 Firebase）——選擇 Google Sheet + Apps Script 是為了沿用使用者已有的免費零維運後端模式，權衡是同步為手動觸發、非即時。
